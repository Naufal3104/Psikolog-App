...
@push('styles')
    <style>
        ...
        .submit-button {
            ...
            padding: 14px 28px; border-radius: 12px; background-color: #eab308;
            color: white; font-weight: 600; border: none; cursor: pointer; margin-top: 1rem;
        }
        
        .submit-button:hover { background-color: #ca8a04; }
        ...
        <div class="content-wrapper">
            ...
            <div class="consultation-card">
                <div class="card-header">
                    <h2 style="margin: 0; font-size: 1.5rem; font-weight: 700;">Edit Artikel</h2>
                </div>

                <form action="{{ route('psikolog.artikel.update', $artikel->id) }}" method="POST" enctype="multipart/form-data" class="card-body" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    @method('PUT')

                    <div class="form-group">
                        <label class="form-label">Judul Artikel</label>
                        <input type="text" name="judul" class="form-input" value="{{ old('judul', $artikel->judul) }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($artikel->gambar)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $artikel->gambar) }}" alt="Current Image" style="height: 100px; border-radius: 8px;">
                            </div>
                        @endif
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="keterangan_gambar" class="form-input" 
                               ...
                               value="{{ old('keterangan_gambar', $artikel->keterangan_gambar) }}">
                        @error('keterangan_gambar') <span style="color:red; font-size:0.8rem;">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Isi Artikel</label>
                        <textarea name="isi" class="form-textarea" required>{{ old('isi', $artikel->isi) }}</textarea>
                    </div>

                    <button type="submit" class="submit-button" :disabled="loading">
                        <span x-show="!loading"><i class="fas fa-save" style="margin-right: 8px;"></i> Simpan Perubahan</span>
                        <span x-show="loading" style="display: none;">Menyimpan...</span>
                    </button>
