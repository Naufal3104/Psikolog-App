    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('kelola-skor.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500 active:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                ...
                <form method="GET" action="{{ route('kelola-skor.index') }}">
                    <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex flex-col md:flex-row justify-between gap-4">
                        <div class="relative flex-grow">
                            {{-- Input Search --}}
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari teks interpretasi..." class="w-full md:w-64 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm pl-10">
                            ...
                            @forelse ($semuaSkor as $item)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 border border-gray-300 dark:border-gray-600">
                                        <div class="font-semibold text-gray-900 dark:text-gray-100">{{ $item->teks_interpretasi }}</div>
                                        {{-- Tampilkan deskripsi singkat jika ada --}}
                                        <div class="text-sm text-gray-500 dark:text-gray-400 truncate">{{ $item->deskripsi_hasil }}</div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 border border-gray-300 dark:border-gray-600 text-center">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            <a href="{{-- route('kelola-skor.edit', $item->id) --}}" title="Edit" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                                            ...
                                            <form action="{{-- route('kelola-skor.destroy', $item->id) --}}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus interpretasi ini?');" class="contents">
                                                @csrf
                                                @method('DELETE')
